...
def draw_2D_grid(ax, grid_bases, num_bins, grid_periods=3):

    if isinstance(grid_periods, int) and grid_periods >= 1:
        pass
    elif isinstance(grid_periods, float) and grid_periods.is_integer() and grid_periods >= 1:
        pass
    else:
        raise Exception("Number of grid periods must be positive integer")

    #if isinstance(grid_periods, int) or isinstance(grid_periods, float) and grid_periods.is_integer():
    #    if grid_periods < 1:
    #        raise Exception("Number of grid periods must be positive integer")
    ...
    xrange = np.arange(-grid_periods, grid_periods + 1 / num_bins, 1 / num_bins)
    yrange = np.arange(-grid_periods, grid_periods + 1 / num_bins, 1 / num_bins)
    ...
    for grid_i in range(grid_bases.shape[0]):
        ...
        for x in xrange:
            ...
            grid_0 = grid_origin + ortho_bases[0] * grid_periods
            grid_1 = grid_origin - ortho_bases[0] * grid_periods
        ...
        for y in yrange:
            ...
            grid_0 = grid_origin + ortho_bases[1] * grid_periods
            grid_1 = grid_origin - ortho_bases[1] * grid_periods
            grid_0 = tuple(grid_0)
            grid_1 = tuple(grid_1)
...
def draw_bases(ax, hgt, colors=None):
    ...
    print("colors:", colors)
    print("scaled_bases:", scaled_bases)
    ...
    for grid_i in range(num_grids):
        ...
        lattice_bases = scaled_bases[grid_i]
        ortho_bases = np.array(np.matmul(lattice_bases, ident))
        V = ortho_bases
        ...
        print("V:", V)
        """
        print("V[:,0]:", V[grid_i,:,0])
        print("V[i,:,1]:", V[grid_i,:,1])
        print("V[i,0,:]:", V[grid_i,0,:])
        print("V[i,1,:]:", V[grid_i,1,:])
        print("V[i,0]:", V[grid_i,0])
        print("V[:,0]:", V[:,0])
        print("V[:,:,0]:", V[:,:,0])
        ...
        print("color:", color)
        print(type(color))
        axes_colors = [color,color]
        print(len(axes_colors))
        print(axes_colors)

        ax.quiver([0,0],[0,0], V[:,0], V[:,1], facecolor=axes_colors, edgecolor=axes_colors, linewidth=0, alpha=0.5, angles='xy', scale_units='xy', scale=1, width=0.015)
        #ax.quiver([0,0],[0,0], V[grid_i,:,0], V[grid_i,:,1], facecolor=axes_colors, edgecolor=axes_colors, linewidth=0, alpha=0.5, angles='xy', scale_units='xy', scale=1, width=0.015)
        #ax.quiver([0,0],[0,0], V[0,:,0], V[0,:,1], facecolor=axes_colors, edgecolor=axes_colors, linewidth=0, alpha=1, angles='xy', scale_units='xy', scale=1, width=0.04)
        #ax.quiver([0,0],[0,0], V[0,:,0], V[0,:,1], axes_colors, facecolor=axes_colors, edgecolor=axes_colors, linewidth=0, alpha=1, angles='xy', scale_units='xy', scale=1, width=0.04)
        #ax.quiver(origin, V[0], V[1], facecolor=color, edgecolor=color, linewidth=0, alpha=1, angles='xy', scale_units='xy', scale=1, width=0.04)
        #ax.quiver(origin, V[:, 0], V[:, 1], color=color, alpha=0.5, angles='xy', scale_units='xy', scale=1)
        ...
        for dim_i in range(num_subspace_dims):
            ...
            ax.plot(points[:, 0], points[:, 1], color=color, alpha=0.5, marker='s')#, clip_on=False)
...
def draw_lines(ax, hgt, ortho_grid_vecs, ortho_grid_angles, points, artists=None, colors=None):
    ...
    for iter_index, pnt in enumerate(points):
        ...
        for dim_i in range(hgt.num_subspace_dims):
            ...
            if artists is None:
                lines = ax.plot([pnt[0], line_x], [pnt[1], line_y], clip_on=True, color=colors[iter_index],
                                alpha=0.3)  # , zorder=6)
